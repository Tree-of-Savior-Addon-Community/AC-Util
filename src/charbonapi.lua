return {
    ...
    GetPostPositionReplacedText = function(self, text)
        ...
        if not tstart then return text end
        -- Add post-position
    ...
    GetStyledText = function(self, text, style)
        ...
        if not style or #style == 0 then
            ...
            local tags = ''
            for i, tag in ipairs(style) do tags = tags .. string.format('{%s}', tag) end
    ...
    Split = function(self, text, delimiter)
        local splitText = {}
        for match in text:gmatch('[^' .. delimiter .. ']+') do table.insert(splitText, match) end
        return splitText
    end,
    ...
    GetValue = function(self, obj, key, delimiter)
        ...
        if not obj or not key or key == '' then return nil end
    ...
    GetLastCharByUnicode = function(self, text)
        if text:len() < 3 then return 0 end
    ...
    HasFinalConsonant = function(self, code)
        if code < 0xAC00 or code > 0xD7A3 then return false end
    ...
    IsExpiredTime = function(self, time, expireTime) return self:GetGameTime() > self:AddTime(time, expireTime) end,
    ...
    TimestampToString = function(self, timestamp)
        ...
        return string.format('%02d/%02d %02d:%02d:%02d', timetable.month, timetable.day, timetable.hour,
    ...
    GetGroupBox = function(self, parent, ctrlName, width, height, left, top)
        -- Handling invalid parameter exceptions
        if not parent or not ctrlName or ctrlName == '' then return nil end
        ...
        top = top or 0
        -- Add group box
        local groupbox = tolua.cast(parent:CreateOrGetControl('groupbox', ctrlName, left, top, width, height),
                             'ui::CGroupBox')
    ...
    GetPicture = function(self, parent, ctrlName, width, height, left, top, image)
        ...
        top = top or 0
        -- Add picture
        local picture = tolua.cast(parent:CreateOrGetControl('picture', ctrlName, left, top, width, height),
                            'ui::CPicture')
    ...
    GetButton = function(self, parent, ctrlName, width, height, left, top, image, tooltip)
        ...
        top = top or 0
        -- Add button
        local button = tolua.cast(parent:CreateOrGetControl('button', ctrlName, left, top, width, height),
                           'ui::CButton')
    ...
    GetCheckBox = function(self, parent, ctrlName, left, top, text, style)
        ...
        top = top or 0
        -- Add check box
        local checkbox = tolua.cast(parent:CreateOrGetControl('checkbox', ctrlName, left, top, 100, 30),
                             'ui::CCheckBox')
