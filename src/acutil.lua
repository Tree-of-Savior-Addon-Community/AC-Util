...
function acutil.getItemRarityColor(itemObj)
    ...
    if (itemProp.setInfo ~= nil) then
        return "00FF00"; -- set piece 
    elseif (grade == 0) then
        return "FFBA03"; -- premium
    elseif (grade == 1) then
    ...
    elseif (grade == 3) then
        return "AA40FF"; -- epic
    elseif (grade == 4) then
        return "FF4F00"; -- Good old Red-ish Legendary that degraded to unique
    elseif (grade == 5) then
        return "EFEA00"; -- All new Bright-Yellow legendary. (Solmiki/Lolo/Some event item. might useful in future)
    end
...
-- end
function acutil.getEventArgs(eventMsg) return unpack(_G['ADDONS']['EVENTS']['ARGS'][eventMsg]); end
...
function acutil.log(msg) CHAT_SYSTEM(acutil.tostring(msg)); end
...
function acutil.slashSet(set)
    ...
    local mainFn = function(words)
        ...
        if (word) then
            for cmd, data in pairs(set.cmds) do
                if (cmd == word) then
                    ...
                    if (qtfound ~= qtexpected) then
                        return executeSetCmd(fnError, set.base .. ' ' .. cmd .. ' expects ' .. qtexpected ..
                                   ' params, not ' .. qtfound .. '.');
                    else
                        local params = {};
                        local n = 0;
            ...
            if (set.empty) then return executeSetCmd(set.empty); end
            local lines = set.base .. '{nl}Show addon help.{nl}-----------{nl}';
            for cmd, data in pairs(set.cmds) do
                local params = ' ';
                local qtparams = data.nparams or 0;
                for i = 1, qtparams do params = params .. '$param' .. i .. ' '; end
...
if not acutil.uiChat_OLD then acutil.uiChat_OLD = ui.Chat; end
...
function ACUTIL_OPEN_ADDON_SYSMENU()
    ...
    for k, v in pairs(ACUTIL_sysmenuAddons) do
        local btn = frm:CreateOrGetControl("button", "acutilAddon" .. tostring(k), status:GetWidth(),
                        status:GetHeight(), ui.LEFT, ui.BOTTOM, 0, margin.top, margin.right, margin.bottom);
...
function ACUTIL_SYSMENU_ICON(frame)
    if acutil.tableLength(ACUTIL_sysmenuAddons) > 0 then
        ...
        local btn = frame:CreateOrGetControl("button", "acutiladdon", extraBag:GetWidth(), extraBag:GetHeight(),
                        ui.LEFT, ui.BOTTOM, 0, margin.top, margin.right, margin.bottom);
        local btnMargin = btn:GetMargin();
...
function SYSMENU_CHECK_HIDE_VAR_ICONS_HOOKED(frame, isAcutil)
    if isAcutil == nil then _G["SYSMENU_CHECK_HIDE_VAR_ICONS_OLD"](frame); end
...
function SYSMENU_CREATE_VARICON_HOOKED(frame, status, ctrlName, frameName, imageName, startX, offsetX, hotkeyName)
    local margin = startX;
    margin = _G["SYSMENU_CREATE_VARICON_OLD"](frame, status, ctrlName, frameName, imageName, startX, offsetX,
                 hotkeyName);
